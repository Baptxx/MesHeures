...
export function HistoryItem({ date, entry, absence, onEdit, onDelete }: Props) {
  const total = entry ? calcDurations(entry).total : null
  const variant = absence ? absence.type : entry && isComplete(entry) ? 'complete' : entry && isStarted(entry) ? 'partial' : 'empty'
        ...
        <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-xs text-zinc-400 dark:text-zinc-500">
          ...
          {absence ? (
            <>
              ...
              <span>{DAY_TYPE_LABELS[absence.type]}</span>
            </>
          ) : entry && (
            <>
              <span className="hidden sm:inline text-zinc-200 dark:text-zinc-700">·</span>
